...
public class DPM implements Drawable {
    ...
    //Condition is the overlap condition. True = overlap, False = no overlap (change to "overlap"?)
    public boolean condition = false;
    //Realroots is the condition that refelcts whether the `Overcond` function computes either two/four real roots (true) or anything else (false)
    public boolean realroots = true;

    // Initialize data structures that characterize the configuration of system
    ...
    public void Overcond(int d, int p) {
        ...
        // Consider the case of two real roots 
        if (nReal == 2) {
        ...
        // Consider the case of four real roots                       
        else if (nReal == 4) {
            realroot1 = realroot_ary[0];
            realroot2 = realroot_ary[1];
        ...
        // Handle the invalid case of neither four nor two real roots
        else {
            realroots = false;
        }
    }
    ...
    //ADD COMMENTS TO THIS FUNCTION NEXT
    public void step() {
